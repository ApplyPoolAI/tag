...
function Avatar({ img, pos, stroke = "#ffffff", size = "big", duration = 20, label }) {
  return (
    <div className={`absolute aspect-square ${pos} z-50 scale-[.6] sm:scale-100`}>
      ...
      <div className={`absolute aspect-square bg-white absCenter rounded-full z-20 ${size === "big" ? 'w-[85%]' : 'w-[90%]'}`}></div>
...
function Dot({ pos }) {
  return (
    <div className={`w-1 h-1 sm:w-3 sm:h-3 bg-dark absolute rounded-full z-10 ${pos}`}>
    </div>
  )
}
...
function Opportunity({ pos, head, sub, loc, img }) {
  return (
    <div className={`absolute z-[100] ${pos} flex items-center gap-2 sm:gap-6 px-2 py-1 sm:px-4 sm:py-2 border sm:border-2 border-sec bg-[#fff] rounded-xl`}>
      <div>
        <img src={img} alt="" className="w-[3vw] aspect-square" />
      </div>
      <div className="">
      <p className="text-[10px] sm:text-base sm:text-[0.7vw] font-light uppercase">{loc}</p>
      <h1 className="sm:py-1 text-[max(1.2vw,12px)] max-w-[14ch] sm:max-w-[14vw] leading-none font-bold">{head}</h1>
      <p className="text-[10px] sm:text-base sm:text-[0.7vw] font-light uppercase">{sub}</p>
      </div>
    </div>
  )
}
        ...
        <Circle pos={"p-[60px] sm:p-[5vw] border-[#00000020]"}>
          ...
          <Circle pos={"p-[25px] sm:p-[10vw] border-[#00000000]"}>
            ...
            <Circle pos={"p-[50px] sm:p-[8vw] border-[#ffffff00]"}>
              ...
              <Avatar pos={"w-[75px] sm:w-[8.5vw] left-[2%] top-[52%] -translate-y-1/2 -translate-x-1/2"} img="./icons/banking.png" stroke="#131E42" label={"Banking"} />
              <Avatar pos={"w-[70px] sm:w-[8vw] right-[12%] top-[84%] -translate-y-1/2 translate-x-1/2"} img="./icons/social science.png" stroke="#131E42" label={"social science"} duration={10} />
              ...
              <Circle pos={"p-[30px] sm:p-[6vw] border-[#ffffff00]"}>
                <div className="w-full h-full absolute rotateCenter border border-[#00000060] rounded-full" style={{ animationDuration: "50s" }}>
                  <Dot pos={"left-[4%] top-[30%] -translate-y-1/2 -translate-x-1/2"} />
                  <Dot pos={"left-[20%] top-[90%] -translate-y-1/2 -translate-x-1/2"} />
                </div>

                <Avatar pos={"w-[70px] sm:w-[7vw] left-[30%] top-[5%] -translate-y-1/2 -translate-x-1/2"} img="./icons/multiple offers.png" stroke="#131E42" label={"Multiple Offers"}/>
